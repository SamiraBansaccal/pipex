...
/*   Updated: 2024/08/15 07:43:39 by sabansac         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	clean_pipex(t_pipex *pipex)
{
	printf("DEBUG: Starting clean_pipex\n");
	...
	if (pipex->is_here_doc)
		unlink(".here_doc_tmp");
	printf("DEBUG: clean_pipex finished\n");
}
...
int here_doc(char *limiter, char *infile)
{
    ...
    printf("DEBUG: Starting here_doc\n");
    fd = open(infile, O_CREAT | O_WRONLY | O_TRUNC, 0644);
    if (fd < 0)
    {
        perror("Here_doc temp file error");
    ...
    printf("DEBUG: Here_doc input complete, opening for reading\n");
    fd = open(infile, O_RDONLY);
    if (fd < 0)
    {
        unlink(infile);
    ...
    printf("DEBUG: Here_doc finished successfully\n");
    return (fd);
}
*/